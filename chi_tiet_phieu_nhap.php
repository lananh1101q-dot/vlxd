...
<head>
    ...
    <title>Chi tiết phiếu nhập - VLXD</title>
    ...
    <style>
        ...
        .main-content{margin-left:0;padding:30px;max-width:900px;margin:30px auto}
...
<div class="main-content">
    <div class="page-header d-flex justify-content-between align-items-center">
        <div>
            <h1 class="mb-1" style="font-size:1.5rem"><i class="fas fa-file-import me-2"></i>Chi tiết phiếu nhập kho</h1>
            <p class="mb-0 opacity-75" id="subTitle">Đang tải...</p>
        </div>
        <a href="danh_sach_phieu_nhap.php" class="btn btn-light"><i class="fas fa-arrow-left me-1"></i>Quay lại</a>
    </div>
    ...
    <div class="card mb-4">
        <div class="card-body">
            ...
            <h6 class="fw-bold text-primary mb-3"><i class="fas fa-list-ul me-1"></i>Chi tiết nguyên vật liệu</h6>
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead><tr><th>Mã NVL</th><th>Tên NVL</th><th>ĐVT</th><th class="text-end">Số lượng</th><th class="text-end">Đơn giá</th><th class="text-end">Thành tiền</th></tr></thead>
                    <tbody id="tbody"><tr><td colspan="6" class="text-center py-3 text-muted">Đang tải...</td></tr></tbody>
                </table>
            </div>
            <div class="d-flex justify-content-end mt-3">
                <div class="bg-success bg-opacity-10 border border-success rounded px-4 py-2 text-success fw-bold">
...
async function load(){
    ...
    try{
        const res=await fetch(API+'/import-receipts/'+encodeURIComponent(id),{headers});
        const data=await res.json();
        if(!data.success)throw new Error(data.message);
        const r=data.data.receipt;
        document.getElementById('subTitle').textContent='Phiếu: '+r.Manhaphang;
        document.getElementById('infoGrid').innerHTML=`
            <div class="info-item"><div class="info-label">Mã phiếu</div><div class="info-value">${r.Manhaphang}</div></div>
            <div class="info-item"><div class="info-label">Nhà cung cấp</div><div class="info-value">${r.Tenncc||r.Mancc||'—'}</div></div>
            <div class="info-item"><div class="info-label">Kho nhập</div><div class="info-value">${r.Tenkho||r.Makho||'—'}</div></div>
            <div class="info-item"><div class="info-label">Ngày nhập</div><div class="info-value">${fmtDate(r.Ngaynhaphang)}</div></div>
        ...
        let total=0;
        document.getElementById('tbody').innerHTML=details.map(d=>{const tt=(d.Soluong||0)*(d.Dongianhap||0);total+=tt;return`<tr>
            <td><code>${d.Manvl}</code></td>
            <td>${d.Tennvl||'—'}</td>
            <td>${d.Dvt||'—'}</td>
            <td class="text-end">${Number(d.Soluong||0).toLocaleString('vi-VN')}</td>
            <td class="text-end">${fmtMoney(d.Dongianhap)}</td>
